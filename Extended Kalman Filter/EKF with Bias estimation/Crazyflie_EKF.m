...
P_check = zeros(15,15,length(t));
P_hat = zeros(15,15,length(t));

% x_check and x_hat are predicted and estimated vectors which are 10x1 at each timestep
x_check = zeros(length(t),16);
x_hat = zeros(length(t),16);
...
for i = 2:length(t)
    P_check(:,:,i) = Predictor(x_hat(i-1,7), x_hat(i-1,8), x_hat(i-1,9), x_hat(i-1,10),...
        ...
        P_hat(:,:,i-1), x_hat(i-1,11), x_hat(i-1,12), x_hat(i-1,13), x_hat(i-1,14),...
        x_hat(i-1,15), x_hat(i-1,16), accel_b_var, om_b_var);
    ...
    [x_check(i,1:3), x_check(i,4:6), x_check(i,7:10)]  = motionModel(x_hat(i-1,1), x_hat(i-1,2), x_hat(i-1,3),...
        ...
        a_x(i), a_y(i), a_z(i), W_x(i), W_y(i), W_z(i), T, x_hat(i-1,11), x_hat(i-1,12), x_hat(i-1,13),...
        x_hat(i-1,14), x_hat(i-1,15), x_hat(i-1,16));
...
%% Modified here, send  X_measure,Y_measure as the measurements 
    ...
    x_hat(i,7:10) = q_hat;
    % update bias
    x_hat(i,11:16) = x_predicted(11:16)+correction(10:15);
...
if (result_plot == true)
    ...
    save('Results/bias.mat','error_x','error_y','error_z','error_vx','error_vy','error_vz',...
